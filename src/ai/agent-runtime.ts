...
import type { JsonRpcNotification, CodexServerRequest } from "./codex-protocol";
import { CodexAppServerClient, defaultBrowserDeveloperInstructions, extractThreadId, modelOption } from "./codex-app-server-client";
...
export class AgentRuntime {
  ...
  constructor(
    ...
    this.client = new CodexAppServerClient({
      ...
      onStatus: (status, message) => {
        this.connection = status === "stopped" ? { status: "stopped" } : { status, ...(message ? { message } : {}) };
  ...
  snapshot(): AgentSnapshot {
    ...
    return {
      ...
      threads: state.threads.map(({ codexThreadId: _codexThreadId, ephemeral: _ephemeral, ...thread }) => thread),
      activeThreadId,
  ...
  private async execute(command: AgentCommand): Promise<void> {
    switch (command.type) {
      ...
      case "agent.thread.rename":
        this.state.updateThread(command.threadId, { title: command.title.trim().slice(0, 80) || "Agent thread" });
        return;
      ...
      case "agent.model.update":
        if (!this.models.some((model) => model.id === command.model)) throw new Error("That model is not available in local Codex");
        this.state.updateThread(command.threadId, { model: command.model, reasoningEffort: command.reasoningEffort });
        return;
  ...
  private async ensureConnection(): Promise<void> {
    ...
    this.models = this.client.availableModels.map(modelOption).filter((model) => model.id !== "unknown");
    this.publish();
  }
  ...
  private async ensureRemoteThread(threadId: string): Promise<string> {
    ...
    const response = await this.client.request<Record<string, unknown>>("thread/start", {
      model: thread.model,
  ...
  private async sendMessage(threadId: string, text: string): Promise<void> {
    ...
    const thread = this.state.getThread(threadId);
    if (!thread) throw new Error("Agent thread not found");
    if (thread.status === "running" || thread.status === "starting" || thread.status === "waiting-for-approval") {
    ...
    this.state.updateThread(threadId, { title: thread.title === "New agent thread" ? safeTitle(message) : thread.title, status: "starting" });
    await this.ensureConnection();
    await this.ensureRemoteThread(threadId);
    await this.sendTurn(threadId, [{ type: "text", text: message }], false);
  }

  private async sendTurn(threadId: string, input: Array<Record<string, unknown>>, continuation: boolean): Promise<void> {
    const thread = this.state.getThread(threadId);
    if (!thread?.codexThreadId) throw new Error("The Codex thread is not ready");
    this.state.updateThread(threadId, { status: "running" });
    const response = await this.client.request<Record<string, unknown>>("turn/start", {
      threadId: thread.codexThreadId,
      input,
      model: thread.model,
      effort: thread.reasoningEffort,
      ...(continuation ? { additionalContext: "The user resumed this browser task after a pause. Continue using the existing browser state." } : {}),
    });
  ...
  private async handleServerRequest(request: CodexServerRequest): Promise<unknown> {
    if (request.method !== "item/tool/call") throw new Error("Only browser dynamic tools are permitted");
    const params = objectValue(request.params);
    const remoteThreadId = stringValue(params.threadId);
    const thread = this.state.getThreads().find((candidate) => candidate.codexThreadId === remoteThreadId);
  ...
  private async requestApproval(threadId: string, input: { actionClass: AgentApprovalRequest["actionClass"]; summary: string; spaceId?: string; tabId?: string }): Promise<boolean> {
    const request: AgentApprovalRequest = { id: randomUUID(), threadId, actionClass: input.actionClass, summary: input.summary, spaceId: input.spaceId, tabId: input.tabId, createdAt: new Date().toISOString() };
    ...
    if (this.state.getThread(threadId)?.status === "waiting-for-approval") this.state.updateThread(threadId, { status: "running" });
    return result;
  }
  ...
  private recordAction(threadId: string, input: { toolName: string; actionClass: AgentApprovalRequest["actionClass"]; spaceId?: string; tabId?: string; summary: string; status: "running" | "succeeded" | "failed" | "denied" }): void {
  ...
  private handleNotification(notification: JsonRpcNotification): void {
    ...
    const remoteThreadId = stringValue(params.threadId);
    const thread = remoteThreadId ? this.state.getThreads().find((candidate) => candidate.codexThreadId === remoteThreadId) : undefined;
    if (!thread) return;
    ...
    if (notification.method === "item/agentMessage/delta") {
      ...
      const existing = this.state.getMessages(thread.id).find((message) => message.id === messageId);
      if (existing) this.state.replaceMessage(messageId, `${existing.text}${delta}`);
      else this.state.appendMessage({ id: messageId, threadId: thread.id, role: "assistant", kind: "text", text: delta });
    ...
    if (notification.method === "turn/completed") {
      ...
      const next = override ?? (status === "interrupted" ? "stopped" : status === "failed" || status === "error" ? "error" : "completed");
  ...
  private parseScopeCommand(threadId: string, message: string): boolean {
    ...
    this.state.appendMessage({ threadId, role: "system", kind: "status", text: "Thread policy updated" });
    return true;
  }
