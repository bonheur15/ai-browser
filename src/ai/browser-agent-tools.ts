...
const success = (...items: ToolContentItem[]): BrowserToolResponse => ({
  success: true,
  contentItems: items,
});
const failure = (message: string): BrowserToolResponse => ({
  success: false,
  contentItems: [{ type: "inputText", text: `Browser action denied or failed: ${message}` }],
});
...
export class BrowserAgentTools {
  ...
  async execute(
    ...
    const toolName =
      request.method === "item/tool/call" && typeof request.params?.tool === "string"
        ? request.params.tool
        : "";
    const normalized = normalizeToolName(toolName);
    ...
    try {
      ...
      if (this.policies.requiresApproval(policy, actionClass)) {
        const approved = await callbacks.requestApproval({
          ...
          ...(target.tab ? { tabId: target.tab.id } : {}),
        });
        if (!approved) {
          callbacks.onAction({
            toolName: normalized,
  ...
  private async run(
    ...
    args: JsonObject,
    policy: AgentPolicy,
    target: { tab?: Tab; spaceId?: string; url?: string },
    callbacks: BrowserAgentCallbacks,
  ): Promise<BrowserToolResponse> {
    switch (normalized) {
      case "list_spaces": {
      ...
      case "list_tabs": {
        ...
        const tabs = snapshot.tabs
          ...
          .filter((entry): entry is { tab: Tab; space: NonNullable<typeof entry.space> } =>
            Boolean(entry.space),
          )
          .filter(({ space }) => space.kind !== "private" || policy.allowPrivate)
          .filter(
            ({ tab }) => !policy.allowedSpaceIds || policy.allowedSpaceIds.includes(tab.spaceId),
          )
      ...
      case "create_tab": {
        ...
        if (!this.policies.originAllowed(policy.allowedOrigins, destinationOrigin(url)))
          return failure("The new tab origin is outside the thread's scope");
        const tab = await this.browser.agentCreateTab(spaceId, url);
        return success({
          type: "inputText",
          text: JSON.stringify({ tab: this.tabSummary(tab, space.name) }),
        });
      }
      case "close_tab":
      ...
      case "clone_tab": {
        ...
        const canCreate = this.policies.canCreateTab(policy, space);
        if (!canCreate.allowed) return failure(canCreate.reason);
        if (!this.policies.originAllowed(policy.allowedOrigins, tab.url))
          return failure("The source tab origin is outside the thread's scope");
        const created = await this.browser.agentCloneTab(tab.id, spaceId);
        return success({
          type: "inputText",
          text: JSON.stringify({
            tab: this.tabSummary(created, space.name),
            loginStateTransferred: false,
          }),
        });
      }
      case "move_tab": {
      ...
      case "navigate": {
        ...
        if (!this.policies.originAllowed(policy.allowedOrigins, destinationOrigin(input)))
          return failure("The destination origin is outside the thread's scope");
      ...
      case "capture_screenshot": {
        ...
        return success(
          {
            type: "inputText",
            text: `Screenshot captured for ${screenshot.title || screenshot.url}`,
          },
      ...
      case "scroll": {
        ...
        return response.ok
          ? success({ type: "inputText", text: JSON.stringify(response.result ?? {}) })
          : failure(response.error ?? "Unable to scroll");
      }
      case "click":
      case "type":
      case "select":
      ...
      case "list_credentials": {
        ...
        const origin = tab
          ? (httpOrigin(tab.url) ?? undefined)
          : optionalText(args.origin, "origin", 500);
        ...
        return success({
          ...
          text: JSON.stringify({
            credentials: credentials.map(
              ({ id, hostname, origin: credentialOrigin, username }) => ({
                id,
                hostname,
      ...
      case "fill_credential": {
        ...
        const credential = this.browser
          .listAgentCredentials(tab.spaceId)
          .find((candidate) => candidate.id === credentialId);
        if (!credential || credential.origin !== httpOrigin(tab.url))
          return failure("That credential does not belong to the active page");
        const ok = await this.browser.agentFillCredential(credentialId, tab.id);
        return ok
          ? success({
              type: "inputText",
              text: "Credential filled locally; the password was not returned",
            })
          : failure("The page did not expose a compatible login form");
      }
      default:
  ...
  private async runPageMutation(
    ...
    if (normalized === "click") {
      ...
      if (!ref && !hasCoordinates)
        throw new Error("click requires an element ref or screenshot coordinates");
      request = {
        type: "click",
        snapshotId: context.snapshotId,
        ...(ref ? { ref } : {}),
        ...(hasCoordinates
          ? {
              x: numberValue(args.x, "x", 0, context.scroll.viewportWidth),
              y: numberValue(args.y, "y", 0, context.scroll.viewportHeight),
            }
          : {}),
      };
      ...
      request = {
        type: "type",
        snapshotId: context.snapshotId,
        ref: text(args.ref, "ref", 80),
        text: text(args.text, "text", 20_000),
        replace: args.replace !== false,
      };
    } else if (normalized === "select") {
      const value = optionalText(args.value, "value", 1_000);
      const label = optionalText(args.label, "label", 1_000);
      request = {
        type: "select",
        snapshotId: context.snapshotId,
        ref: text(args.ref, "ref", 80),
    ...
    return response.ok
      ? success({ type: "inputText", text: JSON.stringify(response.result ?? {}) })
      : failure(response.error ?? "The page action failed");
  }

  private async resolveActionClass(
    normalized: string,
    args: JsonObject,
    target: { tab?: Tab },
    policy: AgentPolicy,
  ): Promise<AgentActionClass> {
    const base = this.actionClass(normalized);
    if (policy.mode !== "guided" || !target.tab || !["click", "press_key"].includes(normalized))
      return base;

    const hasCoordinates =
      normalized === "click" && typeof args.x === "number" && typeof args.y === "number";
    const key = typeof args.key === "string" ? args.key.toLowerCase() : "";
    if (hasCoordinates || (normalized === "press_key" && key === "enter" && !args.ref))
    ...
    if (/\b(delete|remove|discard|cancel|sign\s*out|log\s*out|revoke)\b/.test(label))
      return "destructive";
    if (
      /\b(send|submit|publish|post|buy|purchase|checkout|pay|confirm|save\s+changes|change\s+password|create\s+account|sign\s*in|log\s*in|connect|authorize)\b/.test(
        label,
      )
    )
      return "external-side-effect";
  ...
  private resolveTarget(args: JsonObject): { tab?: Tab; spaceId?: string; url?: string } {
    ...
    const resolvedSpaceId = spaceId ?? tab?.spaceId;
    return {
      ...(tab ? { tab } : {}),
  ...
  private policyDecision(
    ...
    const space = target.spaceId
      ? this.browser.getAgentSpace(target.spaceId)
      : target.tab
        ? this.browser.getAgentSpace(target.tab.spaceId)
        : undefined;
    const policyUrl = target.url
      ? (httpOrigin(target.url) ?? "https://www.google.com")
      : target.tab?.url;
    return this.policies.decide(policy, actionClass, {
      ...(space ? { space } : {}),
      ...(target.tab ? { tab: target.tab } : {}),
      ...(policyUrl ? { url: policyUrl } : {}),
    });
  }
  ...
  private tabSummary(tab: Tab, spaceName: string): JsonObject {
    return {
      id: tab.id,
      spaceId: tab.spaceId,
      spaceName,
  ...
  private summary(tool: string, args: JsonObject, target: { tab?: Tab; spaceId?: string }): string {
    const location = target.tab
      ? ` on ${target.tab.title || target.tab.url}`
      : target.spaceId
        ? ` in Space ${target.spaceId}`
        : "";
    if (tool === "type") return `Type ${String(args.text ?? "").length} characters${location}`;
    if (tool === "navigate")
      return `Navigate ${destinationOrigin(String(args.input ?? ""))}${location}`;
...
const stringSchema = (description: string, maxLength?: number): JsonObject => ({
  type: "string",
  description,
  ...(maxLength ? { maxLength } : {}),
});
...
const toolDefinitions = [
  ...
  {
    name: "list_tabs",
    description: "List tabs available to this thread, including their Space and URL.",
    inputSchema: objectSchema({}),
  },
  {
    name: "create_tab",
    description: "Create a tab in a specific Space. Use an HTTP(S) URL or search text.",
    inputSchema: objectSchema(
      {
        spaceId: stringSchema("Destination Space id"),
        url: stringSchema("HTTP(S) URL or search text", 4_000),
      },
      ["spaceId"],
    ),
  },
  {
    ...
    inputSchema: objectSchema({ tabId: stringSchema("Tab id") }, ["tabId"]),
  },
  {
    name: "activate_tab",
    description: "Make a tab visible to the user.",
    inputSchema: objectSchema({ tabId: stringSchema("Tab id") }, ["tabId"]),
  },
  {
    name: "clone_tab",
    description: "Clone a tab URL into another Space without transferring login state.",
    inputSchema: objectSchema(
      { tabId: stringSchema("Source tab id"), spaceId: stringSchema("Destination Space id") },
      ["tabId", "spaceId"],
    ),
  },
  {
    name: "move_tab",
    description: "Move a tab URL into another Space without transferring login state.",
    inputSchema: objectSchema(
      { tabId: stringSchema("Source tab id"), spaceId: stringSchema("Destination Space id") },
      ["tabId", "spaceId"],
    ),
  ...
  {
    name: "navigate",
    description: "Navigate a tab to an HTTP(S) URL or Google search.",
    inputSchema: objectSchema(
      { tabId: stringSchema("Tab id"), input: stringSchema("URL or search text", 4_000) },
      ["tabId", "input"],
    ),
  },
  {
  ...
  },
  {
    ...
    description:
      "Read bounded, structured page context with safe element references, including detected CAPTCHA widgets and their viewport rectangles.",
    inputSchema: objectSchema({ tabId: stringSchema("Tab id") }, ["tabId"]),
  },
  {
    name: "capture_screenshot",
    description:
      "Capture a redacted screenshot of a tab. Inspect it for controls rendered inside cross-origin iframes such as reCAPTCHA; screenshot-coordinate clicks can activate those controls.",
    inputSchema: objectSchema({ tabId: stringSchema("Tab id") }, ["tabId"]),
  },
  {
    name: "scroll",
    description: "Scroll a page or referenced scrollable element.",
    inputSchema: objectSchema(
      {
        tabId: stringSchema("Tab id"),
        ref: stringSchema("Optional element ref"),
        x: { type: "number" },
        y: { type: "number" },
      },
      ["tabId"],
  ...
  {
    name: "click",
    description: "Click a semantic page element, or bounded screenshot coordinates.",
    inputSchema: objectSchema(
      {
        tabId: stringSchema("Tab id"),
        ref: stringSchema("Element ref"),
        x: { type: "number" },
        y: { type: "number" },
      },
      ["tabId"],
  ...
  },
  {
    name: "type",
    description:
      "Type non-secret text into a page element. Password and payment fields must use the Vault.",
    inputSchema: objectSchema(
      {
        tabId: stringSchema("Tab id"),
        ref: stringSchema("Element ref"),
        text: stringSchema("Text to type", 20_000),
  ...
  {
    name: "select",
    description: "Select an option in a page select menu.",
    inputSchema: objectSchema(
      {
        tabId: stringSchema("Tab id"),
        ref: stringSchema("Select element ref"),
        value: stringSchema("Option value"),
  ...
  {
    name: "press_key",
    description: "Press a keyboard key on a page element.",
    inputSchema: objectSchema(
      {
        tabId: stringSchema("Tab id"),
        ref: stringSchema("Optional element ref"),
        key: stringSchema("Key name", 40),
      },
      ["tabId", "key"],
    ),
  },
  {
    name: "submit_form",
    description: "Submit a page form using a form or submit-control ref.",
    inputSchema: objectSchema(
      { tabId: stringSchema("Tab id"), ref: stringSchema("Form or submit-control ref") },
      ["tabId", "ref"],
    ),
  },
  {
    name: "list_credentials",
    description: "List saved credential summaries without passwords.",
    inputSchema: objectSchema({
      spaceId: stringSchema("Space id"),
      origin: stringSchema("Optional origin"),
    }),
  },
  {
    name: "fill_credential",
    description:
      "Fill a saved credential directly into the matching website form; the password is never returned.",
    inputSchema: objectSchema(
      { tabId: stringSchema("Tab id"), credentialId: stringSchema("Credential id") },
      ["tabId", "credentialId"],
    ),
  },
];
