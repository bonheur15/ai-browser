...
const toolDefinitions = [
  ...
  { name: "get_page_context", description: "Read bounded, structured page context with safe element references, including detected CAPTCHA widgets and their viewport rectangles.", inputSchema: objectSchema({ tabId: stringSchema("Tab id") }, ["tabId"]) },
  { name: "capture_screenshot", description: "Capture a redacted screenshot of a tab. Inspect it for controls rendered inside cross-origin iframes such as reCAPTCHA; screenshot-coordinate clicks can activate those controls.", inputSchema: objectSchema({ tabId: stringSchema("Tab id") }, ["tabId"]) },
