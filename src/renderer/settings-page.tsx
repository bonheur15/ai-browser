...
import type { AppearanceMode, AppSnapshot, BrowserCommand, Space } from "../shared/contracts";
...
export function SettingsPage({
  ...
  const updateAgent = (policy: AgentPolicy): void =>
    onDispatchAgent({ type: "agent.defaults.update", policy });
          ...
          {section === "appearance" && (
            <div className="settings-section-stack">
              <SettingsSectionHeader
                ...
                description="These choices style the AI Browser shell. Websites keep their own appearance settings."
              />
              <SettingsCard
                title="Theme"
                description="Choose how the browser frame should look."
                icon="◐"
              >
                <div className="theme-choice-grid" role="radiogroup" aria-label="Theme mode">
                  {(["dark", "light", "system"] as AppearanceMode[]).map((mode) => (
                    ...
                    >
                      ...
                      <small>
                        {mode === "system" ? "Follow your device" : `${themeLabels[mode]} surfaces`}
                      </small>
                    </button>
                  ))}
                </div>
              ...
              <SettingsCard
                title="Accent color"
                description="Use a single visual signal across actions, focus states, and active surfaces."
                icon="✦"
              >
              ...
              <SettingsCard
                title="Live preview"
                description="A small preview of how your current choices combine."
                icon="▦"
              >
                <div className="settings-preview">
                  ...
                  <div className="settings-preview-body">
                    <div className="settings-preview-rail">
                      <i />
                      <i className="is-active" />
                      <i />
                    </div>
                    <div className="settings-preview-main">
                      <div className="settings-preview-line long" />
          ...
          {section === "workspace" && (
            <div className="settings-section-stack">
              <SettingsSectionHeader
                ...
                description="Spaces keep identities separate. Their colors and icons are independent from the global accent."
              />
              <div className="settings-card settings-space-card">
                <div className="settings-card-heading">
                  <div>
                    ...
                    <div>
                      <h2>Your Spaces</h2>
                      <p>Personal and private contexts currently available in this window.</p>
                    </div>
                  </div>
                  <button className="small-primary" type="button" onClick={onCreateSpace}>
                    + New Space
                  </button>
                </div>
                <div className="settings-space-list">
                  {snapshot.spaces.map((space) => (
                    <SpaceSettingRow
                      ...
                      onAppearance={(color, icon) =>
                        onDispatch({
                          type: "space.updateAppearance",
                          spaceId: space.id,
                          color,
                          icon,
                        })
          ...
          {section === "agent" && (
            <div className="settings-section-stack">
              <SettingsSectionHeader
                ...
                description="These defaults are copied into new Agent threads. Existing threads keep their own policies."
              />
              <SettingsCard
                title="Default mode"
                description="Choose how much autonomy new threads should have."
                icon="✧"
              >
                <div className="agent-mode-grid" role="radiogroup" aria-label="Agent default mode">
                  {(["full", "guided", "observe"] as const).map((mode) => (
                    <button
                      key={mode}
                      className={`agent-mode-choice${agentSnapshot.globalDefaults.mode === mode ? " is-selected" : ""}`}
                      type="button"
                      role="radio"
                      aria-checked={agentSnapshot.globalDefaults.mode === mode}
                      onClick={() => updateAgentField("mode", mode)}
                    >
                      <strong>
                        {mode === "full"
                          ? "Autonomous"
                      ...
                      <small>
                        {mode === "full"
                          ? "Can complete allowed actions"
                          : mode === "guided"
                            ? "Pauses before sensitive actions"
                            : "Reads without changing pages"}
                      </small>
                    </button>
                  ))}
                </div>
              ...
              <SettingsCard
                ...
                description="Keep only the capabilities you want available by default."
                icon="⌁"
              >
                <div className="settings-toggle-list">
                  {agentActions.map(([action, label]) => (
                    <label className="settings-toggle-row" key={action}>
                      <span>
                        <strong>{label}</strong>
                        <small>{action}</small>
                      </span>
                      <input
                        type="checkbox"
                        checked={agentSnapshot.globalDefaults.allowedActions.includes(action)}
                        onChange={(event) =>
                          updateAgentField(
                            "allowedActions",
                            event.target.checked
                              ? [...agentSnapshot.globalDefaults.allowedActions, action]
                              : agentSnapshot.globalDefaults.allowedActions.filter(
              ...
              <SettingsCard
                ...
                description="Additional limits applied to new Agent threads."
                icon="⊙"
              >
                <div className="settings-toggle-list">
                  <ToggleRow
                    label="Use saved credentials"
                    description="Allow explicit Vault fill actions"
                    checked={agentSnapshot.globalDefaults.allowVault}
                    onChange={(value) => updateAgentField("allowVault", value)}
                  />
                  <ToggleRow
                    label="Access Private Spaces"
                    description="Private Spaces remain excluded by default"
                    checked={agentSnapshot.globalDefaults.allowPrivate}
                    onChange={(value) => updateAgentField("allowPrivate", value)}
                  />
                  <label className="settings-range-row">
                    <span>
                      <strong>Maximum tabs</strong>
                      <small>New threads may open up to this many tabs.</small>
                    </span>
                    <input
                      type="number"
                      min={1}
                      max={100}
                      value={agentSnapshot.globalDefaults.maxTabs}
                      onChange={(event) =>
                        updateAgentField(
                          "maxTabs",
                ...
                <div className="agent-connection-note">
                  <span
                    className={`connection-pip connection-${agentSnapshot.connection.status}`}
                  />
                  <span>
                    Agent connection: <strong>{agentSnapshot.connection.status}</strong>
                  </span>
                </div>
              </SettingsCard>
            </div>
          ...
          {section === "shortcuts" && (
            <div className="settings-section-stack">
              <SettingsSectionHeader
                eyebrow="SHORTCUTS"
                title="Move quickly"
                description="The shortcuts currently available across the browser shell."
              />
              <div className="settings-card shortcut-card">
                {[
                  ["⌘ / Ctrl", "L", "Focus the command field"],
                  ["⌘ / Ctrl", "Enter", "Run the Agent composer"],
                  ["⌘ / Ctrl", ",", "Open Settings"],
                  ["Esc", "", "Close Settings outside text inputs"],
              ...
              <ResetPreferences
                onResetAppearance={() => onDispatch({ type: "settings.resetAppearance" })}
                onResetAgent={() => onDispatchAgent({ type: "agent.defaults.reset" })}
              />
            </div>
          )}
        </div>
...
function SpaceSettingRow({
  ...
  onDelete: () => void;
  onAppearance: (color: string, icon: string) => void;
}) {
  const [name, setName] = useState(space.name);
  useEffect(() => setName(space.name), [space.name]);
  return (
    <div className={`settings-space-row${expanded ? " is-expanded" : ""}`}>
      <div className="settings-space-summary">
        <span
          className="settings-space-icon"
        ...
        <button className="settings-space-edit" type="button" onClick={onToggle}>
          {expanded ? "Done" : "Edit"}
        </button>
      </div>
      {expanded && (
        <div className="settings-space-editor">
          <label>
            Name
            <input
              value={name}
          ...
          <div>
            ...
            <div className="space-color-grid">
              {spaceColors.map((color) => (
                <button
                  className={`space-color-option${space.color === color ? " is-selected" : ""}`}
                  type="button"
                  key={color}
                  aria-label={`Use ${color}`}
                  style={{ background: color }}
                  onClick={() => onAppearance(color, space.icon)}
                />
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="space-icon-grid">
              {spaceIcons.map(([icon, label]) => (
                <button
                  className={`space-icon-option${space.icon === icon ? " is-selected" : ""}`}
                  type="button"
                  key={icon}
                  aria-label={label}
                  onClick={() => onAppearance(space.color, icon)}
                >
                  {iconFor(icon)}
                </button>
              ))}
...
function ResetPreferences({
  ...
  const reset = (): void => {
    if (
      !window.confirm(
        "Reset appearance and Agent defaults? Your Spaces, website data, Vault credentials, and Agent history will remain.",
      )
    )
      return;
    onResetAppearance();
  ...
  return (
    <section className="settings-reset">
      <div>
        ...
        <h2>Start fresh with preferences</h2>
        <p>
          Restore Dark + Mint and the default Agent boundaries. This will not delete your browsing
          data.
        </p>
      </div>
